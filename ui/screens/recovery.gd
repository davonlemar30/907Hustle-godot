...
## Recovery — the health meter, the treatment ladder, and REST.
##
...
##
## SO-D1/SO-D3 (1.6.0): the third card was LAY LOW and is now REST. It carries
## canon's same warning, which is still the whole trade: **debt, wages, markets
## and Curtis keep moving while the lights are off.** What changed is that the
## card now previews TWO things — the health a slot of sleep puts back, and,
## only while today's quiet is unspent, the Heat it also sheds. A second REST in
## one day previews health alone, because that is all it will do.
...
func _rest_card(sys: Object) -> Control:
	var c := card()
	var v := VBoxContainer.new()
	v.add_theme_constant_override("separation", 5)
	c.add_child(v)
	...
	head.add_theme_constant_override("separation", 8)
	v.add_child(head)
	var t := label("REST", "CardTitle", 13, CREAM)
	t.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	head.add_child(t)
	head.add_child(label("NEXT PART OF DAY", "Mono", 10, MUTED))

	# Both halves, and only the halves that are actually on offer. The quiet is
	# once a day, so a second REST previews health alone rather than promising
	# Heat it will not shed.
	var health_back: int = int(sys.rest_health_preview())
	var heat_off: float = float(sys.rest_quiet_preview())
	var promise: String = ""
	if health_back > 0 and heat_off > 0.0:
		promise = "Expected immediate result: %d Health back, and Heat down by %s." \
			% [health_back, _heat_text(heat_off)]
	elif health_back > 0:
		promise = "Expected immediate result: %d Health back. Today's quiet is already spent." % health_back
	elif heat_off > 0.0:
		promise = "Expected immediate result: Heat down by %s. Nothing left to sleep off." % _heat_text(heat_off)
	else:
		promise = "Nothing to sleep off."
	v.add_child(label(
		"%s Debt, wages, markets, and Curtis continue moving while the lights are off."
			% promise, "Muted", 11, MUTED, true))
	# The blocker is the label, the same as every other card in the build.
	var blocked: String = str(sys.rest_blocker())
	var b := button("REST" if blocked.is_empty() else blocked.to_upper(),
		false, _on_rest, 44)
	b.disabled = not blocked.is_empty()
	v.add_child(b)
	v.add_child(label("Restores health and advances time", "Muted", 10, MUTED))
	return c
...
func _on_rest() -> void:
	var sys: Object = _recovery()
	var quiet: bool = sys != null and bool(sys.quiet_available())
	if _gm.dispatch("rest", {}):
		if quiet:
			nav.show_toast("Quiet night. Health %d/%d, Heat %.1f." % [gs.health, gs.health_max, gs.heat])
		else:
			nav.show_toast("You lie back down. Health %d/%d." % [gs.health, gs.health_max])
