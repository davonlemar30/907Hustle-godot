...
| Parity | **12,591 checks, 0 failures**, floor `MIN_CHECKS := 12591` — includes the merged stickup-caught decision coverage and the suite-level developer-save guard |
...
| Confrontation suite | **250 checks, 0 failures** — a CI gate as of the rooms build (`tests/confrontation/`), the loop's own harness on the shared territory asserts; includes both Lift and stickup caught-decision paths |
...
| Save validation | **235 checks, 0 failures** — a CI gate as of batch 12; covers nested save shapes through v24 and asserts that catalogue validation releases its temporary GameState Node |
