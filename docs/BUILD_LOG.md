...
Every dated batch and phase entry from `HANDOFF.md`'s history, moved here whole
in Batch 18 PR 5 because the file they came from was 6,690+ lines with four
different sort orders and the one table a new reader actually needs sat near
the bottom of it.

**Ordering: preserved from the source file, not re-sorted by date.** `HANDOFF.md`
was already *roughly* newest-first at the macro level (recent batches near the
top, Phase 3-era entries near the bottom) but not strictly so — several entries
were appended out of chronological order relative to their neighbors, which is
part of what PR 5's audit found. Attempting a full chronological re-sort here
would mean guessing at exact dates for entries that only carry a day-level
timestamp, and a wrong guess is worse than an honestly-ordered append history:
this file's order is the order these entries were actually WRITTEN into the
project's documentation, which is itself a real and verifiable fact, unlike a
reconstructed "true" chronology this session cannot fully verify.

**What is not here:** the Build 5e divergences and the Divergence Protocol
(`docs/DECISIONS.md` — a standing ruling, not a dated narrative), and every
`D-` numbered ruling (`docs/DECISIONS.md`). Where a batch entry below
established a ruling that file now owns, this file keeps the narrative and
points at the ADR entry rather than duplicating it.

For current, living reference — the design system, canon tables, working
notes, and the last few batches — see `HANDOFF.md`. For standing rulings, see
`docs/DECISIONS.md`.
...
### Standing Policy — Build 5e divergences, and the Divergence Protocol

Moved to `docs/DECISIONS.md` in Batch 18 PR 5 — these are permanent rulings the ADR file now owns, not narrative that belongs in a dated batch entry.
...
## Retro — Batches 6b + 7: crew stops being decorative, and two rooms open  (retro entry added 2026-08-23, Batch 18 PR 5)

**Written retroactively.** These two batches shipped with build-log rows only —
no narrative section — which the standing PROCESS rule in `HANDOFF.md` calls
retro-documentation and says not to do. Written now because Batch 18 PR 3
migrates state these batches' era introduced, and a migration across
undocumented state is harder to reason about than one across documented state.
The reasoning below is reconstructed from the build-log summary and the
shipped code, not from a live session — some of the near-misses and discarded
alternatives PROCESS says retro-documentation loses are genuinely gone.

**Batch 6b — Tone, Eli and Deshawn do something.** Three crew members had been
recruitable since Phase 3e with no mechanical effect: loyalty tracked, wages
charged, and nothing else read the roster. This batch gave each one exactly one
job. Tone absorbs damage at both sites that deal it (Stickup and Boost's Caught
path) through a single `absorbed_damage()` call, deliberately the one place his
multiplier is consumed — the same "once" contract Deshawn's Heat multiplier
later formalized in FS-003.9. Eli covers a carry (`run_the_bag`, reducing
Boost's stop chance) and Deshawn works a corner (`smooth_it_over`, District
Pressure relief) — both delegated operations, the pattern FS-001.7 later built
Pherris's board on top of. Schema unchanged: nothing here needed a new
persisted field, because loyalty and wage tracking already existed and this
batch only gave existing state new READERS.

**Batch 7 — two venue interiors.** Spenard Gym and Night Owl, the first rooms
in the build a player walks INTO rather than an action they dispatch from a
menu. `effectiveAttribute` and the gym streak are ported from canon; the
`night_owl` job — previously authored but unreachable — becomes findable
through the counter. Save v10 → v11: `recovery_introduced`-adjacent venue state
(gym sessions, streak, entered venues) is additive, and the migration arm's own
comment already says why defaulting it in is correct rather than a fallback —
a v10 save genuinely never had a gym to walk into.

Both batches are "give a shipped system its second half" work: 6b gave
recruited crew a reason to be recruited beyond a stat block, 7 gave an
authored-but-dead job a door.

## Retro — Batches 8 + 9: Heat gets an edge, and the instrument gets a class fix  (retro entry added 2026-08-23, Batch 18 PR 5)

**Written retroactively**, for the same reason as the 6b/7 entry above.

**Batch 8 — Heat gets teeth at both ends.** Before this batch Heat was a number
that went up; nothing read it as a THREAT. Heat bands gave it named zones, the
quiet-day decay gave a clean day a reward, and the street stop gave the top of
the scale a real consequence. A propagation-order inversion (Exposure reading
Heat before the night's own fold landed) was found and fixed in the same pass —
the ordering-matters lesson `day_lifecycle.gd` would later generalize into a
declared contract rather than a per-bug fix. Save v11 → v12: the two fields
`heat_gain_today` and `lay_low_day` are additive, and the migration arm's own
reasoning (a v11 save has no record either way, so the generous default costs
the player at most one decay it might not have earned) is the same shape of
argument every additive arm in this chain makes.

**Batch 9 — the instrument was lying.** The most consequential batch in this
group and the reason "report, do not tune" became the standing rule the rest
of this build repeats. A leaked product catalogue had been corrupting every
economy measurement since batch 3: `reset_to_new_game()` restored none of the
authored catalogue fields, so a check that swapped one polluted every profile
measured after it in the same process, and the pollution read as a real result
for six batches. Found, the class of bug was closed (catalogues are snapshotted
once and restored on every reset, not just the fields any one check happened to
think to reset), and two balance findings were filed with live numbers rather
than fixed on the spot — the first appearance of the pattern Batch 18's own
escalations (`docs/DECISIONS.md`) still follow: a finding with numbers attached
is worth more than a guessed fix. Schema unchanged — this was purely a harness
bug, which is itself worth noting: the ECONOMY was never wrong, the INSTRUMENT
measuring it was.

## Retro — Batches 10 + 11: Wander ships, then survives an adversarial read  (retro entry added 2026-08-23, Batch 18 PR 5)

**Written retroactively**, for the same reason as the entries above.

**Batch 10 — Wander.** The fourth consequence-chain kind (after Stickup, Boost
and Retaliation) and the first action that costs a slot to find something
rather than to earn directly. Ramped discovery (base chance rising with
misses, capped), a card registry (`data/wander_events.gd`) separating the
draw's data from the draw's mechanics, and three of Home's buttons that had
been decorative since the screen shipped became live. Two previously-authored
jobs that had no discovery path became reachable through it. Save v12 → v13:
`jobs_discovered`/`boost_targets_discovered`-adjacent wander state, additive,
same reasoning shape as every arm before it.

**Batch 11 — an adversarial read of batch 10 found five shipped defects.** Not
a new feature batch — a batch whose entire content is auditing the batch before
it, on the theory that a system that just shipped is the one most likely to
have something wrong nobody has looked for yet. All five found defects were
fixed, adapter-supplied choice copy was added (the pattern that would later
generalize into `list_adapter.gd`'s domain-adapter shape), and the glyph
coverage CI job was extended to scan `data/` — Wander's card registry is
exactly the kind of file a glyph regression could hide in unnoticed, being
data rather than UI code. Schema unchanged: every fix here was behavioral, not
a new field.

The two together are the clearest early example of the pattern Batch 18 itself
follows: ship a slice, then immediately audit it before building the next
thing on top, rather than trusting a green suite that has never been asked an
adversarial question.

## Retro — Batches 12 + 13: Wander gets measured, then gets a shape  (retro entry added 2026-08-23, Batch 18 PR 5)

**Written retroactively**, for the same reason as the entries above.

**Batch 12 — Wander measured.** The first batch to ask what Wander is actually
worth rather than whether it works: 307% of the day job, the discovery-to-money
mechanism pinned as an assertion rather than left as an unmeasured feature, and
— notably — the OTHER two harnesses (save-validation, screen-smoke) gated into
CI for the first time, rather than remaining manual-only checks a developer had
to remember to run. That last change is the direct ancestor of Build 18 PR 0's
own CI work: a gate nobody runs automatically is not a gate.

**Batch 13 — Wander becomes a choice.** Before this batch, Wander was one
button with one outcome shape. This batch added three intents (distinct player
choices about what a walk is FOR) and a per-day effort falloff — the same
diminishing-value shape Territory's soldier-stacking curve uses, applied here
to repeated walks in a day rather than repeated soldiers on a corner. READ, one
of the three intents, is singled out in the build log as "the intent that
tells you what the build hides" — a walk that surfaces information about
systems the player has not engaged yet, rather than money or a discovery
directly. Save v13 → v14: `wanders_today`, additive and zero-defaulted, the
same reasoning every prior arm in this chain uses — a save from before the
field existed has no walks to recover, so zero is the honest history rather
than a fallback standing in for one.

Read together with batches 10-11, this is Wander's full arc in the build:
ship it, adversarially audit it, measure what it is worth, then give it enough
internal structure that "go for a walk" is a decision with a shape rather than
a single undifferentiated button.
