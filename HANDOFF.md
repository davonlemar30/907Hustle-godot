...
_Last updated: 2026-09-07 (1.6.0). Living doc — update as screens land._
...
| | |
| --- | --- |
| Build version | `1.6.0` — Sleep It Off: REST, the night, the severe band (`autoload/version.gd`) |
| Save schema | **v36** — `damage_today` (SO-D4, 1.6.0), how much health today has taken off you. Additive: a v35 save loads with 0, which reads as "today has not hurt yet". **Derived, not reported**: `reconcile_persistent_invariants()` keeps a runtime-only `_health_seen` and adds any DROP on every dispatch, so the fourteen damage writers stay untouched. Heals never subtract. Eight bumps across 1.0.0–1.6.0, every one additive: v29 `rent_arrears_day` (OG-D1), v30 `weapon`/`vehicle`/`trunk` (OG-D3), v31 `game_over_kind`/`leaving`/`run_earnings` (OG-D4), v32 `hot_goods` (OG-D5), v33 `curtis_dismantled`/`curtis_dismantle_hold` (HS-D3), v34 `market_nudges` (TU-D4), v35 `businesses` (HSS-D2), v36 `damage_today` (SO-D4). Migration arms are documented by version in `autoload/save_system.gd`'s `SAVE_VERSION` header. |
| Parity | **14,909 checks, 0 failures**, floor `MIN_CHECKS := 14909` (`tests/parity/parity_runner.gd`). Set from a fresh run at every PR. |
...
| Save validation | **342 checks, 0 failures** (`tests/save_validation/save_validation_runner.gd`). **This runner has no `MIN_CHECKS` constant** — the count is asserted by the CI step, not by a floor. |
| Screen smoke | Every screen instantiated **at the phone's width (375)** over the longest lines the game writes: **1,260/1,260 touch checks**, **2,941/2,941 width checks** (no visible control outside the viewport — BR-D1, the stretch bug's regression test), **56/56 component checks**, **133/133 panel checks**. `MIN_UNCOVERED := 0.35` in `tests/smoke/screen_smoke.gd` is a card-legibility ratio, not a check-count floor. |
...
| Recovery | **Free is slow, paid is fast, and zero is still death** (D-35, 1.6.0). **REST** — no money, one slot, **+10**, no per-day cap (time is the cap) and no district gate; refused only when the run is over or there is nothing to sleep off. The **first REST of a day** also sheds 2.0 Heat through `apply_relief` and files Curtis's `quiet_day` — **Lay Low was folded into REST** and deleted whole, and `lay_low_day` keeps its name because renaming a persisted field is a schema change. **The night** (`recovery_overnight`, second in `DAY_START_ORDER`) heals **+3** after a damage-free day, **+1** at or below the **severe band of 30**, and nothing after a day that hurt; `overnight_amount()` is the one function that decides a night's value, left as the gate a future injury state will need. **The paid ladder is untouched**: first aid +18/$55/no slot, clinic +40/$135/one slot at ≤82, doctor +75/$290/one slot at ≤55 behind Mina's TRUSTED band. From 5 health with $0, three RESTs clear the band in a day and resting reaches full health in three — against 26 nights on the night alone. |
...
| Rulings | Latest recorded is **D-35** (Sleep It Off, SO-D1..D6). **D-36 is the next free number — re-verify against `docs/DECISIONS.md` before claiming it.** |
...
| Latest PRs | **1.6.0** — three PRs plus the close-out: sleep it off (SO-D1/D3/D4, `#183`), the night (SO-D2/D5, `#184`), a harness that heals (SO-D6, `#185`), the close-out. Before that: **1.5.1** — the floor (`#180`), the first week (`#181`), the close-out (`#182`). Before that: **1.5.0** — `#176`–`#179`; **1.4.0** — `#171`–`#174`; **1.3.0** — `#166`–`#170`; 1.2.0 His Side of the Board (D-30), 1.1.0 Somebody Shows You Around (D-29), 1.0.0 One Good Run (D-28). Assessments: `docs/ONE_GOOD_RUN_REVIEW.md`, `docs/BLOCK_REMEMBERS_REVIEW.md`, `docs/WORLD_SPEAKS_REVIEW.md`, `docs/VISION_REVIEW.md`. |
