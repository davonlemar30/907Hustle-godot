...
| Screen smoke | 23/23 screens instantiate **with their scripts attached**, **1101/1101 touch checks**, and — new in 0.6.0 — **61/61 component checks**. `ModalSheet` had shipped for two builds with nothing asserting it could be constructed at all; the suite now builds it, `encounter_sheet.gd` and `health_bar.gd` against a REAL live chain (not a stub summary) and runs the same TOUCH-D5 scroll-transparency walk over the result. That gap caught a real parse error inside a minute during PR A |
